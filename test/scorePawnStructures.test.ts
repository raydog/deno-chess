...
Deno.test("Board Scoring > Pawn Structures > White doubled pawns", function () {
  const b = boardFromFEN("8/1P1P1P2/P2P1PP1/PP2P1P1/P1P1PP1P/1P4P1/1PP1P1P1/8 w - - 0 1");
  const score = scorePawnStructures(b, _config({
    DoubledPawn: -10
  }));
...
Deno.test("Board Scoring > Pawn Structures > Black doubled pawns", function () {
  const b = boardFromFEN("8/1pp2p2/p1p2p2/p1pp2pp/1p3p2/1p3p1p/p1p2p2/8 w - - 0 1");
  const score = scorePawnStructures(b, _config({
    DoubledPawn: -10
  }));
...
function _config(pawnConfig: Partial<ScoreSettings["PawnStructure"]>): ScoreSettings {
  return {
    ...settings,
    PawnStructure: {

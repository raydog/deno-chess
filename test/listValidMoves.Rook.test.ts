...
Deno.test("List Valid Moves > Rook > Center movements", function () {
  const b = boardLayout({
    d5: encodePieceSpace(PieceType.Rook, COLOR_WHITE, true),
  });
...
Deno.test("List Valid Moves > Rook > Handles blocks", function () {
  const b = boardLayout({
    h7: encodePieceSpace(PieceType.Rook, COLOR_WHITE, true),
    e7: encodePieceSpace(PieceType.King, COLOR_WHITE, true),
    h4: encodePieceSpace(PieceType.Knight, COLOR_WHITE, true),
  });
...
Deno.test("List Valid Moves > Rook > Handles captures", function () {
  const b = boardLayout({
    d7: encodePieceSpace(PieceType.Rook, COLOR_WHITE, true),
    d5: encodePieceSpace(PieceType.Rook, COLOR_BLACK, true),
  });

...
import { SPACE_EMPTY, spaceGetType } from "../src/core/datatypes/Space.ts";
import {
  PIECETYPE_BISHOP,
  PIECETYPE_KING,
  PIECETYPE_PAWN,
...
Deno.test("Build Chess960 > Build board matches spec", function () {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    for (let file = 0; file < 8; file++) {
      asserts.assert(
        spaceGetType(b.get(0x10 | file)) === PIECETYPE_PAWN,
        "Non-pawn on white pawn row",
      );
      asserts.assert(
        spaceGetType(b.get(0x60 | file)) === PIECETYPE_PAWN,
        "Non-pawn on black pawn row",
      );

      const whiteIdx = 0x00 | file, blackIdx = 0x70 | file;
      asserts.assert(
        b.get(whiteIdx) !== SPACE_EMPTY,
        "Space on white back row is empty",
      );
      asserts.assert(
        b.get(blackIdx) !== SPACE_EMPTY,
        "Space on black back row is empty",
      );
      ...
      if (spaceGetType(b.get(whiteIdx)) === PIECETYPE_KING) {
        asserts.assert(
          rookNum === 1,
          `There were ${rookNum} rooks to the left of the King`,
        );
      }
    }
    ...
    asserts.assert(
      rookNum === 2,
      "Didn't have 1 rook to the right of the King",
    );
  }
});
